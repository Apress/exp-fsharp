...
<script language="F#" runat="server">
    member page.GenerateData_Click(sender: obj, e: EventArgs) =
        let isPrime(i:bigint) =
            let lim = Math.BigInt.FromInt64(int64(sqrt(float(i))))
            let rec check j =
                j > lim or (i % j <> 0I && check (j+1I))
            check 2I

        let lowerLimit = Math.BigInt.Parse(page.LowerLimit.Text)
        let upperLimit = Math.BigInt.Parse(page.UpperLimit.Text)
        let data = 
