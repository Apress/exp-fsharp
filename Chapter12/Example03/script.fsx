...
let truthTable  x = 
    x |> support |> Set.to_list |> cases |> Seq.map (fun env -> env,eval env x)

let satisfiable x = 
    x |> truthTable |> Seq.exists(fun (env,res) -> res)

let tautology x = 
    x |> truthTable |> Seq.for_all (fun (env,res) -> res)
...
let printCounterExample = 
    (function None -> printfn "tautology verified OK"
            | Some env -> printfn "tautology failed on %A" (Seq.to_list env))

let stringOfBit b = (if b then "T" else "F")
let stringOfEnv env = 
    Map.fold(fun k v acc -> sprintf "%s=%s;" k (stringOfBit v)+acc) env ""
...
let AndL l = Seq.fold1 (fun x y-> And(x,y)) l 
