...
and ExprSize =
    ...
    member self.Expand (exprs: VisualElement list) =
        List.fold_left ExprSize.ExpandOne self exprs

    static member Seq (exprs: VisualElement list) =
        List.fold_left ExprSize.ExpandOne ExprSize.Zero exprs
...
type VisualExpr =
    ...
    static member OfExpr (opt: RenderOptions) e =
        ...
        and exp (opt: RenderOptions) prec = function
            | Num n ->
                let s = BigNum.to_string n in Symbol (s, sizeOf opt s)
            | Var v ->
...
//  Listing 12-11.
    ...
    member self.Render =
        ...
        let rec draw (gra: Graphics) opt x y psize = function
            ...
            | Sequence (exps, size) ->
                let x', y' = size.CenterOnMidline psize x y
                List.fold_left (fun (x, y) (e: VisualElement) ->
